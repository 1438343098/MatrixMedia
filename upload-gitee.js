...
function resolveUploadPaths() {
  ...
  const raw = path.join(buildDir, package.build.productName + ' Setup ' + name + '.exe');
  ...
  const req = https.request(options, res => {
    ...
    res.on("end", () => {
      if (res.statusCode === 201) {
        ...
        console.error("创建失败:", res.statusCode, data);
...
function tgaGetRelease(callback) {
  const options = {
    hostname: baseUrl,
    path: createReleaseApi + "/tags/" + pushBody.tag + "?access_token=" + access_token,
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      // 可能还需要其他标头，比如授权信息
...
function getAllReleases(callback) {
  const options = {
    hostname: baseUrl,
    path: createReleaseApi+'?page=1&per_page=100&access_token='+access_token,
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      // 可能还需要其他标头，比如授权信息
...
function deleteOldestRelease() {
  getAllReleases(releases => {
    ...
    if (releases.length >= 5) {
      ...
      const options = {
        hostname: baseUrl,
        path: createReleaseApi +`/${oldestRelease.id}?access_token=${access_token}`,
        method: "DELETE",
      };
      const req = https.request(options, res => {
