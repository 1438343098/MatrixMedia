...
!macro customUnInstall
  ...
  nsExec::ExecToLog 'powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = ''Stop''; $dir = [System.IO.Path]::GetFullPath(''$INSTDIR''); $path = [Environment]::GetEnvironmentVariable(''Path'', ''User''); if ([string]::IsNullOrWhiteSpace($path)) { exit 0 }; $parts = $path -split ''\;'' | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '''' -and $_ -ne $dir }; $newPath = ($parts | Select-Object -Unique) -join ''\;''; [Environment]::SetEnvironmentVariable(''Path'', $newPath, ''User'')"'
  Pop $0
